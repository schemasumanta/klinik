  ...
  <div class="content">
    <div class="page-inner">
      <div class="page-header">
        <h4 class="page-title">TAMPILAN PASIEN SEDANG RAWAT INAP</h4>
                ...
                <table  id="tabel_ranap_sedang_periksa"  class="table table-striped table-bordered table-sm " style="width: 100%; height: 30%; overflow-y: scroll;overflow-x: scroll; font-size: 13px; text-align: left;">
                  <thead>
                    <tr style="background: #03b509 ;text-align: left; color:white">
                      ...
                      <th data-urut="tanggal_berobat">Tgl Daftar</th>  
                      <th data-urut="status_pasien">Status Pasien</th> 
...
<script type="text/javascript"> 
  ...
  let url_sedang_periksa ="<?php echo base_url()?>ranap/ambil_data_sedang_periksa_farmasi?sortdata="+sortdata_sedang+"&filterdata="+sortdata_sedang;
  ...
  $(document).ready(function(){
    ...
    $('.sorting').on('click',function(){
      ...
      url_sedang_periksa="<?php echo base_url()?>ranap/ambil_data_sedang_periksa_farmasi/"+sort_sedang+"/"+filter_sedang
      tabel_sedang_periksa.ajax.url("<?php echo base_url()?>ranap/ambil_data_sedang_periksa_farmasi?sortdata="+sort_sedang+"&filterdata="+sort_sedang).load();
    });
  ...
  $('#show_data1').on('click','.item_list_observasi', function(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        '<th data-urut="status_pasien">Status Pemberian Obat</th>'+ 
        '<th style="text-align: center;" data-urut="kode_ranap">Option</th>'+
        '</tr>'+
        '</thead>'+
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          '<td><div class="btn-group"><a href="javascript:;" class="btn btn-success btn-flat btn-sm item_detail_observasi" style="font-weight: bold;"  data-status="'+data[i].status_pemberian_obat+'"  data-observasi="'+data[i].kode_observasi+'" data-ranap="'+data[i].kode_ranap+'"><i class="fa fa-eye"></i></a></div></td>'+
          '</tr>';
        }
  ...
  $('#ModalObservasi').on('click','.item_detail_observasi', function(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        html+='<td colspan="6" class="text-right"><a href="<?php echo base_url(); ?>ranap/update_pemberian_obat/'+kode_observasi+'" class="btn btn-primary btn-sm">Sudah Diberikan</a></td>'+ 
        '</tr>';
        }
        html+='</tbody></table></div>';
